<script>
	...
	/* const formatData = (contents) => {
		contents.forEach((element) => {
			let contentList = element.data.content;
			contentList.reduce((p, v) => {}, []);
		});
	};
	formatData(data.data.results);
		...
		preformatted: ({ node }) => {
			...
			return `
				<div class="mockup-code">
					${list.map((item,i)=>{	return `<xmp class="" data-prefix="$">${item}</xmp>`})}
				</div>
			`
		},
  ...
  `;
		...
		embed: ({ node }) => `
        <div id="youtubeEmbed" class="w-full" data-oembed="${node.oembed.embed_url}"
